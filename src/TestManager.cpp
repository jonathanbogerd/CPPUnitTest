...
void TestManager::add_test(const std::string &group_name, const std::string &test_name,
...
void TestManager::run_all_st_tests() {
...
void TestManager::run_all_mt_tests() {
    ...
    for (const auto &[name, test]: mt_tests) {
        semaphore.acquire();
        futures.push_back(
            std::async(std::launch::async, [&name, &test, &semaphore, &output_mutex, &group_width,
                                            &test_width, &mt_failed_tests]() {
                try {
                    {
